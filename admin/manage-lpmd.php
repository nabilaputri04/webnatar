...
if (isset($_POST['simpan'])) {
    ...
    if ($nama === '' || !in_array($jabatan, $jabatan_opts, true)) {
        ...
        if ($id_edit > 0) {
            $sql = "UPDATE lpmd SET nama='{$nama}', jabatan='{$jabatan}', foto='{$foto}', urutan={$urutan} WHERE id={$id_edit}";
            if (mysqli_query($conn, $sql)) $success = "Data LPMD diperbarui."; else $error = mysqli_error($conn);
        } else {
            $sql = "INSERT INTO lpmd (nama, jabatan, foto, urutan) VALUES ('{$nama}','{$jabatan}','{$foto}',{$urutan})";
            if (mysqli_query($conn, $sql)) $success = "Anggota LPMD ditambahkan."; else $error = mysqli_error($conn);
        }
    }
    header('Location: manage-lpmd.php'); exit;
}
